...
function home() {
  return shell(`<section class="screen">
    ...
    ${state.answered ? '' : `<section class="question"><button class="question-meal" data-action="detail"><span class="mini-photo">${foodPlate()}</span><span><strong>${t('lunch')}</strong><small>13:05 · 612 ${t('kcal')}</small></span><b aria-hidden="true">›</b></button><p class="question-label">✦ ${t('question')}</p><p class="question-text">${t('questionText')}</p><div class="composer"><input id="answer" placeholder="${t('answer')}"/><button class="send-circle" data-action="answer" aria-label="${t('answerAction')}">↑</button></div></section>`}
...
function detail() {
  ...
  return shell(`<section class="screen detail"><header class="header">${btnIcon('back','Back','home')}<h2>${t('details')}</h2><button class="header-action" data-action="edit">${t('edit')}</button></header>
    ...
    ${state.answered ? '' : `<section class="question detail-question"><p class="question-label">✦ ${t('question')}</p><p class="question-text">${t('questionText')}</p><div class="composer"><input id="detail-answer" placeholder="${t('answer')}"/><button class="send-circle" data-action="answer" aria-label="${t('answerAction')}">↑</button></div></section>`}
    ...
    <section class="ai-fix"><p class="ai-label">✦ ${t('fix')}</p><div class="composer"><input id="fix" placeholder="${t('fixPlaceholder')}"/><button class="send-circle" data-action="fix" aria-label="${t('sendCorrection')}">↑</button></div></section>
  </section>`);
}
